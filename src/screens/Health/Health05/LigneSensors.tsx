import React from 'react';
import {StyleProp, TouchableOpacity, View, ViewStyle} from 'react-native';
import {
  StyleService,
  useStyleSheet,
  Layout,
  Icon,
  useTheme,
} from '@ui-kitten/components';

import Text from 'components/Text';
import ProgressBar from 'components/perso/Ligne';
import numeral from 'numeral';


interface Props {
  title: string;
  cals: number;
  totalCals: number;
  style?: StyleProp<ViewStyle>;
}

const LigneSensors = ({title, cals, totalCals, style}: Props) => {
  const styles = useStyleSheet(themedStyles);
  const theme = useTheme();
  const formatNumber = (amount: number) => {
    let textResult = ``;
    const _amount = amount.toFixed(2);
    try {
      if (isNaN(parseFloat(_amount))) {
        textResult += numeral(parseFloat(_amount.replace(',', ''))).format(
          '0,0',
        );
      } else {
        textResult += numeral(parseFloat(_amount)).format('0,0');
      }
    } catch (e) {
      console.log(e);
    }
    return textResult;
  };
  return (
    <Layout style={[styles.container, style]} level="9">
      <Text category="h6" children={title} marginBottom={17} />
      <ProgressBar
        progress={cals / totalCals}
        progressColor={theme['color-success-100']}
        containColor={`${theme['color-success-100']}20`}
      />
      <View style={styles.calsView}>
        <Text category="h6">{formatNumber(cals)}</Text>
        <Text category="h7" opacity={0.5}>
          {formatNumber(totalCals)}
        </Text>
      </View>
      <TouchableOpacity style={styles.btnShowMore}>
        <Icon
          pack="assets"
          name="caret_down"
          style={styles.icon}
          opacity={0.7}
        />
      </TouchableOpacity>
    </Layout>
  );
};

export default LigneSensors;

const themedStyles = StyleService.create({
  container: {
    borderRadius: 20,
    paddingTop: 0,
    paddingHorizontal: 16,
    marginHorizontal: 24,
  },
  calsView: {
    flexDirection: 'row',
    marginTop: 4,
    justifyContent: 'space-between',
    marginBottom: 10,
  },
  btnShowMore: {
    alignItems: 'center',
  },
  icon: {
    tintColor: 'text-basic-color',
    marginBottom: 12,
  },
});